...
async def test_cleanup_old_spend_logs_batch_deletion():
    ...
    # First 3 calls hit SpendLogs.
    spend_logs_sql = mock_db.execute_raw.call_args_list[0][0][0]
    assert 'DELETE FROM "LiteLLM_SpendLogs"' in spend_logs_sql
    assert 'WHERE "request_id" IN' in spend_logs_sql

    # Next 3 calls hit SpendLogToolIndex.
    tool_index_sql = mock_db.execute_raw.call_args_list[3][0][0]
    assert 'DELETE FROM "LiteLLM_SpendLogToolIndex"' in tool_index_sql
    assert 'WHERE "ctid" IN' in tool_index_sql
