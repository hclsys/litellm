"""
Regression tests for streaming per-chunk overhead optimizations (LIT-3313).

Covers all 3 providers (Anthropic, Bedrock Invoke, Bedrock Converse) across
the key behavioural edge cases introduced by the hot-path changes.
...
| usage chunk stripped from output  | ✓         | ✓              | ✓                |
| usage preserved in hidden_params  | ✓         | ✓              | ✓                |
| finish_reason propagated          | ✓         | ✓              | ✓                |
| sync path matches async path      | ✓         | –              | –                |
| model_dump() NOT called per chunk | ✓         | –              | –                |
| _GCHUNK_FIELDS module constant    | ✓         | –              | –                |
| callback list resolved once       | ✓         | –              | –                |
"""
...
def test_sync_path_no_model_dump_on_text_chunks():
    ...
    Before the fix, hasattr(response, "usage") was always True for
    ModelResponseStream (because usage is a declared field), causing an
    expensive model_dump() + model_response_creator() call on every chunk.
    """
    chunks = [
    ...
    # model_dump should NOT have been called for simple text chunks
    # (It is called at end-of-stream for stream_chunk_builder, but not per-chunk)
    ...
    # model_dump calls should be at most 1 (for stream_chunk_builder at the end)
    # — crucially not N calls (one per text chunk)
    assert model_dump_call_count <= 1, (
        f"model_dump() called {model_dump_call_count} times — "
        "hot-path regression: usage check is firing on every chunk"
    )
