#!/usr/bin/env python3
"""Benchmark CustomStreamWrapper per-chunk overhead (LIT-3313).

Targets the optimizations in PR #28720, which all live inside
``CustomStreamWrapper`` and reduce work executed on every streamed chunk:

  1. Fix sync ``__next__`` regression — ``hasattr(response, "usage")`` was
     always True so ``model_dump()`` ran on every text chunk.
  2. Cache effective model name + logging provider at ``__init__``.
  3. Pre-compute ``_base_hidden_params`` once.
  4. Cache resolved streaming callbacks for the stream lifetime.
  5. Module-level ``_GCHUNK_FIELDS`` frozenset (no per-call
     ``__annotations__`` read).

A full proxy benchmark like ``benchmark_anthropic_messages_perf.py`` would
include FastAPI, the HTTP stack, and TCP latency — which dilutes the signal
from per-chunk CPU work.  This script drives ``CustomStreamWrapper``
directly with synthetic in-memory chunks, mirroring the three providers the
PR cares about (Anthropic GenericStreamingChunk, Bedrock Invoke
GenericStreamingChunk, Bedrock Converse ModelResponseStream).
...
# ---------------------------------------------------------------------------
# Synthetic chunk fixtures — keep in lock-step with test_streaming_overhead.py
# ---------------------------------------------------------------------------
...
def bedrock_invoke_chunks(n: int) -> List[GChunk]:
    # Bedrock invoke also surfaces GChunk-shaped dicts through the
    # CustomStreamWrapper hot path — identical synthetic shape to Anthropic.
