#!/usr/bin/env python3
"""Tight microbenchmark for CustomStreamWrapper.model_response_creator (LIT-3313).

Most of the per-chunk cost in PR #28720 lives inside model_response_creator:

  * Cached effective model name (no dict lookup + prefix string-format per call)
  * Pre-computed _base_hidden_params (one dict spread per call instead of two)

Driving the full wrapper (sync __next__ / async __anext__) adds threadpool
logging, gc, and other noise that swamps these microsecond-scale savings.
This benchmark calls model_response_creator() in a tight loop on a pre-built
wrapper to isolate the change.
